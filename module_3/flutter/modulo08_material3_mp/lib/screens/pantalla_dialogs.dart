...
class PantallaDialogs extends StatelessWidget {
  ...
  void _mostrarMensaje(BuildContext context, {bool esError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo mostrart la informacion'
            : 'Hola como estas soy Dylan Fernandez'),
        backgroundColor: esError
            ? Theme.of(context).colorScheme.error
            : null,
        action: SnackBarAction(
  ...
  Future<void> _datosFormulario(BuildContext context) async {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Libro "${ctrlNombre.text}" agregado')),
      );
    }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Registrar lector'),
        content: Form(
          key: formKey,
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              TextFormField(
                controller: ctrlIp,
                decoration: const InputDecoration(labelText: 'Código de carné'),
                validator:  (v) {
                  if (v == null || v.isEmpty) return 'Campo requerido';
                  final partes = v.split('.');
                  if (partes.length != 4) return 'Formato: L-10293';
                  return null;
                },
              ),
            ],
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Lector "${ctrlNombre.text}" registrado')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            label: const Text('SnackBar de información'),
          ),
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar libro (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Registrar lector (formulario)'),
          ),
          const SizedBox(height: 8),
          OutlinedButton(
            onPressed: () => _datosFormulario(context),
            child: const Text('Agregar libro (formulario)'),
          ),
        ],
      ),
    );
